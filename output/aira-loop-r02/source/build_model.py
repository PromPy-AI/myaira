"""AIRA Loop R02: deterministic concept geometry, not released production CAD.
...
add('sensor','U2 · PPG + optical baffle',solid(10,14,2,1.3,3.1),'#25352d',1,'Reserved 10 × 14 × 1.8 mm volume for optical module and black isolation baffle. MAX30101 candidate. Scheduled optical windows for the 72-hour target; no continuous-HR claim.')
add('pcb','PCB1 · Custom PCB envelope',solid(23,42,4,3.4,4.2),'#427d65',2,'23 × 42 × 0.8 mm four-layer PCB planning envelope. Components and RF region are allocation blocks, not a routed PCB.')
add('mcu','U1 · BLE controller',solid(7,7,.3,4.2,5.3,0,5),'#283239',2,'Nordic nRF52840 candidate. No AI inference. PDM/DMA, light ADPCM framing, encryption and BLE only. See the engineering sheet for proposed GPIO assignments.')
add('flash','U5 · Reconnect buffer',solid(8,6,.4,4.2,5.3,-5,-6),'#2d3435',2,'Winbond W25Q512JV family candidate, 512 Mbit / 64 MiB NOR. About 1.70 h at the 70 kbps budget after 20% reserve. Phone is the main store. Current and driver need validation.')
add('pmic','U6 · Power management',solid(5,5,.3,4.2,5.4,6,-10),'#383d36',2,'Nordic nPM1300 candidate; battery charger/power path and proposed 3V0/1V8 rails. Reference circuit and physical pad map remain unverified.')
add('imu','U3 · Accelerometer',solid(3,3,.3,4.2,5.2,7,5),'#7a8874',2,'Bosch BMA400 candidate. Accelerometer only; no gyro. Proposed low-power motion mode must be selected and measured.')
add('temp','U4 · Temperature island',solid(3,3,.3,1.4,2.2,8,-12),'#81948a',1,'TI TMP117 candidate on a thermally isolated flex island near the skin. Not a core body temperature measurement.')
...
add('antenna','RF clearance region',solid(15,4,1,4.25,4.35,0,19),'#c29e61',2,'Indicative antenna keepout, y = 17 to 21 mm. Reserve all layers and nearby enclosure space per the chosen antenna reference layout. Gold region is not a copper design.','rf')
add('ledpower','U7 · LED power supply',solid(3,3,.3,4.2,5.4,6,-4),'#7e8873',2,'TPS63031 candidate buck-boost for optical LED rail. Magnetics and reference circuit are not yet placed or validated.')
...
add('battery','Protected LiPo envelope',solid(18,28,1.4,6.8,10.8,0,-1),'#c8c6b6',3,'18 × 28 × 4 mm allocation including assumed protection/tab allowance. 230 mAh is an unverified capacity target within the 200–230 mAh constraint. Supplier drawing must confirm both fit and capacity.')
add('mic1','MIC1 · Single microphone',solid(4,3,0.4,9.7,11.1,0,15),'#969c8d',4,'TDK/InvenSense ICS-41351 PDM candidate. One outward-facing microphone; exact current, availability and pinout need verification. Nominal allocation clears battery and RF region by 0.5 mm in Y. Flex and gasket need further allowance; acoustic testing is required.')
...
model={'revision':'R02','units':'mm','body':{'width':27,'length':48,'height':13},'parts':parts}
...
def export_stl(path, mesh):
    ...
    with path.open('wb') as out:
        out.write(b'AIRA R02 FORM FIT BLANK - mm - NOT PRODUCTION'.ljust(80,b' '));out.write(struct.pack('<I',len(f)))
        for face in f:
...
lines=['# AIRA Loop R02; millimetres; conceptual component envelopes'];offset=1
for p in parts:
    lines.append('o '+p['id'])
